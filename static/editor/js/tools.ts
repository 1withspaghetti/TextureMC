...
class PaintBucket implements Tool {
    ...
    onMouseDown(e: MouseEvent, pos: {x: number, y: number}, canvas: Canvas) {
        ...
        if (target[0] == c.r && target[1] == c.g && target[2] == c.b && target[3] == c.a * 255) return; // Same color detection
        this.spread(canvas.main, canvas.color, pos.x, pos.y, target);
        canvas.saveHistory();
    }

    onMouseMove(e: MouseEvent, pos: {x: number, y: number}, canvas: Canvas) {
        if (!canvas.active) setPixel(canvas.highlight, null, pos.x, pos.y);
    }

    onMouseUp(e: MouseEvent, pos: {x: number, y: number}, canvas: Canvas) {
    }

    spread(ctx: CanvasRenderingContext2D, color: tinycolor.Instance, x: number, y: number, target: Uint8ClampedArray) : void {
    ...
    check(a: Uint8ClampedArray, b: Uint8ClampedArray): boolean {
...
class LineTool implements Tool {
    ...
    onMouseMove(e: MouseEvent, pos: {x: number, y: number}, canvas: Canvas) {
        if (canvas.active) {
            ...
            if (e.shiftKey) pos = squareCoords(this.lastPosition, pos);
            for(let pixel of canvas.toolPixels) {
                drawLine(ctx, null,
                    this.lastPosition.x + pixel.x, this.lastPosition.y + pixel.y,
                    pos.x + pixel.x, pos.y + pixel.y)
        ...
        else drawCircleFromPixels(canvas.toolPixels, canvas.highlight, null, pos.x, pos.y);
    }
...
class VariationTool {
    appliedPixels: {x: number, y: number}[] = [];
    ...
    pixelIncludes(list: {x: number, y: number}[], pixel: {x: number, y: number}): boolean {
        for (let test of list) {
            if (test.x == pixel.x && test.y == pixel.y) return true;
        }
        return false;
    }

    randomize(d: Uint8ClampedArray): Uint8ClampedArray {
        if (Math.random() * 2 > 1) {
...
function squareCoords(c1: {x: number, y: number}, c2: {x: number, y: number}): {x: number, y: number} {
...
function drawLine(ctx: CanvasRenderingContext2D, color: tinycolor.Instance | null, x1: number, y1: number, x2: number, y2: number): void {
...
function drawOval(x0: number, y0: number, x1: number, y1: number): Array<{x: number, y: number}> {
    var c = {
        x0 : Math.min(x0, x1),
        y0 : Math.min(y0, y1),
        x1 : Math.max(x0, x1),
    ...
    var pixels: Array<{x: number, y: number}> = [];
...
function drawCircle(diameter: number, ctx: CanvasRenderingContext2D, color: tinycolor.Instance | null, centerX?: number, centerY?: number): Array<{x: number, y: number}> {
...
function drawCircleFromPixels(pixels: Array<{x:number,y:number}>, ctx: CanvasRenderingContext2D, color: tinycolor.Instance | null, centerX: number, centerY: number): void {
    for (let pixel of pixels) {
        setPixel(ctx, color, centerX + pixel.x, centerY + pixel.y);
    }
}

function setPixel(ctx: CanvasRenderingContext2D, color: tinycolor.Instance | null, x: number, y: number) {
    if (!color) {
...
function generateCircle(diameter: number): Array<{x: number, y: number}> {
    if (diameter == 1) return [{x: 0, y: 0}]; // Shortcut for speed

    var radius = diameter / 2;

    var maxblocks_x: number, maxblocks_y: number;
